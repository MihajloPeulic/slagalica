...
import { ChevronRight, Loader2, Swords, Trophy, UserPlus, UserRound, X } from "lucide-react";

import { useRouter } from "next/navigation";

import { AddAFriend } from "@/actions/friends";
import { createGameRoom } from "@/actions/game/game";
...
export default function FriendsModal() {
    ...
    const { isUserOnline, presenceReady } = useOnlinePresence();
    ...
    const [inviteActiveRoomId, setInviteActiveRoomId] = useState<string | null>(null);
    ...
    async function handleInvite(friendId: string) {
        ...
        setIsInviting(true);
        setInviteError(null);
        setInviteActiveRoomId(null);

        try {
            const res = await createGameRoom(friendId);

            /*
                Ako JA već imam aktivnu partiju,
                createGameRoom vraća activeRoomId.

                OVO MORA biti prije res.error provjere,
                jer server namjerno može vratiti i:
                {
                    error: "...",
                    activeRoomId: "..."
                }
            */
            if (res?.activeRoomId) {
                setInviteActiveRoomId(res.activeRoomId);
                setInviteError("Već imaš aktivnu partiju.");

                return;
            }

            /*
                Ako je PRIJATELJ već zauzet, nema activeRoomId
                i prikazujemo samo običnu poruku.
            */
            if (res?.error) {
            ...
            if (!res?.roomId) {
                setInviteError("Nije moguće kreirati partiju.");
            ...
            setInviteError("Došlo je do greške. Pokušaj ponovo.");
        } finally {
            setIsInviting(false);
    ...
    const onlineCount = friends.filter((friend) => presenceReady && isUserOnline(friend.id)).length;
                ...
                <header className="border-b border-border p-4">
                    <div className="mb-4 flex items-start justify-between gap-4">
                        <div>
                            ...
                            <h2 className="section-title">Prijatelji</h2>
                        </div>

                        {activeTab === "friends" && <OnlineCounter count={onlineCount} />}
                    </div>

                    <div className="grid grid-cols-2 gap-1 rounded-xl bg-background p-1">
                        <TabButton active={activeTab === "friends"} onClick={() => handleTabChange("friends")}>
                            Prijatelji
                        </TabButton>

                        <TabButton active={activeTab === "add"} onClick={() => handleTabChange("add")}>
                            Dodaj
                        </TabButton>
                    </div>
                </header>
...
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
    return (
        ...
        font-black
        transition-colors
        ${active ? "bg-surface-light text-text" : "text-text-secondary hover:text-text"}
      `}
        >
            {children}
        </button>
            ...
            {friends.map((friend) => {
                ...
                return (
                    <FriendRow
                        ...
                        presenceReady={presenceReady}
                        isInviting={isInviting}
                        initial={getInitial(friend.username)}
                        onToggle={() => onSelectFriend(selected ? null : friend.id)}
...
function FriendRow({
    ...
    return (
        ...
        border
        transition-colors
        ${selected ? "border-primary/40 bg-primary/5" : "border-border bg-background hover:bg-surface-light/40"}
      `}
        >
            <button type="button" onClick={onToggle} className="group flex w-full items-center gap-3 p-3 text-left">
                <FriendAvatar initial={initial} online={online} selected={selected} />
            ...
            ${selected ? "rotate-90 text-primary" : "text-text-secondary"}
          `}
                />
            </button>
...
function FriendAvatar({ initial, online, selected }: { initial: string; online: boolean; selected: boolean }) {
    return (
        <div className="relative shrink-0">
          ...
          text-sm
          font-black
          ${selected ? "border-primary/40 bg-primary/10 text-primary" : "border-border bg-surface-light text-text"}
        `}
            >
                {initial}
            </div>
    ...
    return (
        <div className="border-t border-border bg-background p-2">
            <div className="grid grid-cols-[1fr_1fr_auto] gap-2">
                <Button type="button" variant="secondary" size="sm" disabled={loading} onClick={onOpenProfile}>
                    <UserRound className="h-4 w-4" />
                    Profil
                </Button>

                <Button type="button" size="sm" disabled={loading} onClick={onInvite}>
                    {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Swords className="h-4 w-4" />}

                    {loading ? "Čekaj..." : "Izazovi"}
                </Button>
...
function EmptyFriends() {
    return (
        <div className="flex min-h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-border px-6 text-center">
            ...
            <p className="card-title">Lista je prazna</p>
...
function AddFriendTab({
    ...
    return (
        <div className="flex flex-col gap-4">
            <div>
                <h3 className="card-title">Pronađi igrača</h3>
